...
public class OfflocService {
    ...
    @Autowired
    public OfflocService(OffenderRepository offenderRepository, AlertsTransformer alertsTransformer, MovementsTransformer movementsTransformer, OffenderProgrammeProfileTransformer offenderProgrammeProfileTransformer, AttendanceAndExclusionTransformer attendanceAndExclusionTransformer, CourtEventsTransformer courtEventsTransformer, EmploymentsTransformer employmentsTransformer, SentenceTransformer sentenceTransformer, SentenceCalculationsTransformer sentenceCalculationsTransformer, DiaryDetailService diaryDetailService, ChargesService chargesService, IEPService iepService, OffenderTransformer offenderTransformer) {
    ...
    public Optional<OffenderCDE> getFullFatOffenderByOffenderId(Long offenderId) {
        ...
        maybeOffender.map(offender -> {
            final ExternalMovement lastSequentialMovement = lastSequentialMovementOf(movements);
            return OffenderCDE.builder()
                    .activeAlerts(activeAlertsOf(maybeMainBooking))
                    .activeTransfers(activeTransfersOf(maybeMainBooking))
                    .activityDetails(activityDetailsOf(maybeMainBooking))
                    ...
                    .highestRankedOffence(highestRankedOffenceOf(charges))
    ...
    private Charge highestRankedOffenceOf(List<Charge> charges) {
        ...
        return mostSeriousCharges.size() == 1 ?
                mostSeriousCharges.get(0) :
                charges.stream().filter(c -> "A".equals(c.getChargeStatus())).findFirst().orElse(null);
    }
    ...
    private EarliestSentenceAndConvictionCDE earliestSentenceAndConvictionOf(Optional<OffenderBooking> maybeMainBooking) {
        ...
        var earliestConvictionForBooking = maybeMainBooking.map(offenderBooking -> offenderBooking.getCourtEvents()
                .stream()
    ...
    private Optional<SentenceCalculation> mostRecentSentenceCalculationForBookingOf(Optional<OffenderBooking> maybeMainBooking) {
        var x = maybeMainBooking.flatMap(offenderBooking -> offenderBooking.getOffenderSentCalculations()
                .stream()
    ...
    private List<ActivityDetailCDE> activityDetailsOf(Optional<OffenderBooking> maybeMainBooking) {
